...
async def create_note(title: str, content: str, tags: list[str] | None = None) -> Note:
    """Create and persist a new note. This should be used whenever something useful about a
    user is learned that you may want to remember in the future. Remember useful details and
    summarize it to be useful in the future. This call is relevenat in all conversations."""
    return project.create_note(title, content, tags)


@app.retrieve
async def get_note(note_id: str) -> Note:
    """Retrieve a single note by its identifier. This is used to remember detailed information about
    a user from previously in this or a different conversation."""
...
async def list_notes(page: int = 1, page_size: int = 10) -> list[NoteSummary]:
    ...
    that have details and memories about the user. It's typically a good idea to use this if
    you think there may have been relevant information or you can personalize based on the
    conversation or question using past memory."""
